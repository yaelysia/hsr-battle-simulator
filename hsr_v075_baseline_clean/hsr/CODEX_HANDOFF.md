...
- P8-S9 至 P8-S11 遗器定义、正式实例骨架、主词条合法池和精确数值。
...
光锥轨已经推进至 R2 并验收，`P8-S9` 至 `P8-S11` 的遗器定义、实例和主词条阶段也已验收。
当前顺序上的下一阶段是 `P8-S12` 副词条所属池、roll 和升级合法性；S12 至 S17 继续按
...
- 副词条合法性仍 deferred 到 S12，因此非空遗器构筑可完成主词条装配，但不能进入正式战斗。
...
- 当前 P8-S12 执行卡：
  `docs/p8_execution_cards/P8-S12_RELIC_SUB_AFFIX_ROLLS.md`。
