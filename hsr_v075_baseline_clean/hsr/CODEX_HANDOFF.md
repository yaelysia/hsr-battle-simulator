...
当前已建立 P8 光锥、遗器与角色构筑装配计划，P8-S0 至 P8-S8 已通过独立验收。P8 的架构方向是：角色卡、光锥卡、遗器卡作为并列内容来源，玩家构筑引用具体实例，L2 构筑装配器生成静态属性贡献、已启用机制和来源账本，runtime 只消费装配结果。S1 已退役 P4 的字符串式装备边界并建立类型化结构；S2 已接通角色晋阶成长、能量上限、行迹 / 星魂子来源、统一贡献账本和正式 scenario 准入；S3 已将当前已发布光锥完整投影为 source-backed 类型化定义目录并接入 Canonical IR / RuleBook；S4 已建立玩家光锥实例、精确成长贡献、命途激活决策、战斗准入和队伍实例唯一性；S5 已将 575 条真实光锥静态属性按命途激活决策接入统一贡献账本，并闭合贡献身份、叠影来源、光锥归属和账本联合伪造边界；S6 已将 162 条真实能力记录接入 Canonical IR 图引用，精确绑定 407 条装备参数读取，并通过正式场景构筑链建立语义唯一、原子且可快照恢复的 provider 启动边界；S7 已闭合属性、状态、条件、动态值与监听机制族；S8 已闭合其余伤害、治疗、护盾、资源、生命、时间线、目标、RNG 和共享战斗状态机制。当前 162 张已发布光锥机制图全部 executable，148 张已正式启动；14 张记忆命途光锥只因角色侧 owned-combatant/忆灵构筑缺口保持外部阻断。遗器尚未 lower。
...
后续依赖采用两轨结构：S5-S8 光锥轨已验收收口，当前从 S9 开始推进 S9-S17 遗器轨。遗器轨内部严格串行、逐阶段 `ready_for_review`、验收和提交。S18 是硬汇合点，只有 S17 验收并与当前光锥检查点合并后才能开始；S18-S21 恢复严格串行。记忆角色与忆灵 owned-combatant 构筑继续由独立角色任务补齐，P8 最终全目录正式入战声明必须等待该外部依赖关闭。
...
```text
当前接续已验收的 P8-S8 检查点。先读 CODEX_HANDOFF、架构边界合同、P8 计划、执行卡索引以及 S8 报告；下一阶段只执行遗器轨 P8-S9。记忆角色与忆灵 owned-combatant 构筑按独立角色任务推进。执行线程只能交 ready_for_review，不得修改 P8 唯一 checklist、提交 Git 或自动进入下一阶段。
```
