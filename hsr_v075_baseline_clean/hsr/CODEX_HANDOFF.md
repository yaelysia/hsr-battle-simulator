...
依赖目标、事件、伤害、状态、资源、波次或随机领域生产者的 operand 继续由 S5、S9-S15 负责，
因此当前正式构筑仍会诚实阻止未闭合角色出生。共享 graph 只构建一次，角色卡只引用；
...
执行卡入口位于 `simulator_v8_clean_core/docs/p9_execution_cards/README.md`。按用户指示，当前在
P9-S4 检查点后暂停；未获得明确继续指令前不得开始 P9-S5 或任何后续阶段。
