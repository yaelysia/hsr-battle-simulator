...
- P8-S9 至 P8-S12 遗器定义、正式实例骨架、主副词条合法池和精确数值。
...
光锥轨已经推进至 R2 并验收，`P8-S9` 至 `P8-S12` 的遗器定义、实例和主副词条阶段也已验收。
当前顺序上的下一阶段是 `P8-S13` 外圈 / 内圈套装档位统计；S13 至 S17 继续按
...
- S12 已完成副词条准入，但套装统计和遗器静态贡献仍未装配，因此非空遗器构筑仍不能进入正式战斗。
...
- 非空合法遗器只形成只读 selection，并以一对一 `relic_assembly` blocker 等待 S13 套装统计和 S14 静态贡献；零效果遗器不能进入正式战斗。
...
- 当前 P8-S13 执行卡：
  `docs/p8_execution_cards/P8-S13_RELIC_SET_THRESHOLDS.md`。
- 已验收 P8-S12 报告：
