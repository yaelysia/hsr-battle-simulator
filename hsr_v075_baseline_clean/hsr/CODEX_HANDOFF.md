...
- P8-S9 至 P8-S13 遗器定义、正式实例骨架、主副词条合法池、精确数值和套装档位统计。
...
光锥轨已经推进至 R2 并验收，`P8-S9` 至 `P8-S13` 的遗器定义、实例、词条和套装统计阶段也已验收。
当前顺序上的下一阶段是 `P8-S14` 遗器与套装静态属性装配；S14 至 S17 继续按
...
- S13 已完成套装档位统计，但遗器静态贡献仍未装配，因此非空遗器构筑仍不能进入正式战斗。
...
- 非空合法遗器只形成只读 selection；S13 已完成套装统计，当前一对一 `relic_assembly` blocker 只等待 S14 静态贡献，零效果遗器不能进入正式战斗。
...
- 当前未应用套装静态属性或启动 ability，非空遗器构筑只因静态贡献尚未装配而保持 battle blocked。
...
- 当前 P8-S14 执行卡：
  `docs/p8_execution_cards/P8-S14_RELIC_STATIC_CONTRIBUTIONS.md`。
- 已验收 P8-S13 报告：
  `live_validation_reports/P8-S13_RELIC_SET_THRESHOLDS_ready_for_review.md`。
