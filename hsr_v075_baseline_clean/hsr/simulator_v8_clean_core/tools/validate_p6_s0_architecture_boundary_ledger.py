...
AUDIT_RULES: tuple[AuditRule, ...] = (
    ...
    AuditRule(
        ...
        tokens=("def character_dynamic_value_bindings_for_card", "bindings = card.source.evidence.get(\"character_config_dynamic_value_bindings\")", "return _json_object_copy(bindings)"),
        classification="audit_only",
        followup_stage="P6-S5",
        current_behavior="RuleBook exposes a copied dynamic-binding view for character cards, centralizing the current source-evidence bridge behind one accessor.",
        boundary_reason="This is an explicit RuleBook boundary used to keep runtime systems from walking source evidence; the remaining cleanup is to project the bindings into a stable IR slot.",
        required_next_action="Keep this accessor narrow and copy-returning; track first-class data-card dynamic binding projection as post-P6 backlog.",
    ),
    AuditRule(
