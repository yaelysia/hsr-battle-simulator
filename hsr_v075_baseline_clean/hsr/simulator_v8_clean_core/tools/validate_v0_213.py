...
from ..rules.ir import ActionDefinitionIR, CanonicalIR, EffectIR, IRSource, TriggerIR
...
def run_validation(
    ...
    if write_full_ir:
        write_json(output_dir / "canonical_ir_v0_213.json", ir.to_json())
    write_json(output_dir / "canonical_ir_summary_v0_213.json", _canonical_ir_summary(ir))
    write_json(output_dir / "coverage_matrix_v0_213.json", coverage.to_json())
    write_json(output_dir / "fidelity_matrix_v0_213.json", fidelity.to_json())
    ...
    lifecycle_cases = _effect_lifecycle_cases(rules, base_state, command, sample)
    ...
    checks = {
        ...
        "risk_guards": _risk_guard_checks(action_transition, scope_case, duplicate_add_modifier_result),
        "status_lifecycle": _status_lifecycle_checks(duplicate_add_modifier_result),
        "remove_modifier": _remove_modifier_checks(lifecycle_cases.get("remove_transition")),
        "fixed_effects": _fixed_effect_checks(lifecycle_cases),
        "unsupported_effect_contract": _unsupported_lifecycle_effect_checks(lifecycle_cases),
        "effect_transition_contracts": _effect_transition_contract_checks(lifecycle_cases),
    }
    ...
    write_json(output_dir / "validation_summary_v0_213.json", result)
    write_json(output_dir / "sample_trigger_executor_transition_v0_213.json", action_transition.to_json())
    write_json(output_dir / "sample_pre_status_application_v0_213.json", _effect_result_json(pre_status_result))
    if unsupported_effect_transition is not None:
        write_json(output_dir / "sample_unsupported_effect_transition_v0_213.json", unsupported_effect_transition.to_json())
    if blocked_condition_transition is not None:
        write_json(output_dir / "sample_blocked_condition_transition_v0_213.json", blocked_condition_transition.to_json())
    if status_ledger_transition is not None:
        write_json(output_dir / "sample_status_ledger_transition_v0_213.json", status_ledger_transition.to_json())
    if non_attack_case.get("transition") is not None:
        write_json(output_dir / "sample_non_attack_transition_v0_213.json", non_attack_case["transition"].to_json())
    if scope_case.get("transition") is not None:
        write_json(output_dir / "sample_scope_mismatch_transition_v0_213.json", scope_case["transition"].to_json())
    write_json(output_dir / "sample_duplicate_add_modifier_v0_213.json", _effect_result_json(duplicate_add_modifier_result))
    for name in ("remove", "heal", "shield", "resource_delta"):
        transition = lifecycle_cases.get(f"{name}_transition")
        if transition is not None:
            write_json(output_dir / f"sample_{name}_effect_transition_v0_213.json", transition.to_json())
    write_json(output_dir / "sample_unsupported_lifecycle_effects_v0_213.json", _lifecycle_case_summary(lifecycle_cases))
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("validation_outputs_v0_213"))
...
def _effect_lifecycle_cases(
    ...
    if sample:
        add_result = _execute_effect(rules, base_state, sample["pre_effect"], command, source_suffix="lifecycle_remove_setup")
        add_state = reducer.apply_all(base_state, add_result.mutations)
        status_instance = _status_instance(add_result)
        owner_id = str(status_instance.get("owner_id") or command.actor_id) if status_instance else command.actor_id
        status_id = str(status_instance.get("status_id") or "") if status_instance else ""
        remove_effect = _synthetic_effect(
            "RemoveModifier",
            "remove_modifier",
            {"target_alias": "ParamEntity", "status_id": status_id},
        )
        remove_result = _execute_effect_with_context(
            registry,
            add_state,
            remove_effect,
            command,
            source_suffix="remove_modifier",
            owner_id=owner_id,
            param_entity_id=owner_id,
            current_action_target_id=owner_id,
        )
        remove_after = reducer.apply_all(add_state, remove_result.mutations)
        cases["remove_result"] = remove_result
        cases["remove_before_state"] = add_state
        cases["remove_transition"] = _effect_transition(
            command=command,
            before_state=add_state,
            after_state=remove_after,
            effect=remove_effect,
            effect_result=remove_result,
            target_id=owner_id,
            coverage_id="v0_213_remove_modifier",
        )

        chance_effect = _synthetic_effect(
            "AddModifier",
            "chance_unsupported_add_modifier",
            {
                "modifier_name": str(sample["pre_effect"].payload["standard"].get("modifier_name") or ""),
                "target_alias": "Caster",
                "chance": {"kind": "fixed", "value": 0.5},
            },
        )
        cases["unsupported_chance_result"] = _execute_effect_with_context(
            registry,
            base_state,
            chance_effect,
            command,
            source_suffix="unsupported_chance",
        )

    damaged_state = _with_damaged_actor(base_state, command.actor_id)
    heal_effect = _synthetic_effect("Heal", "fixed_heal", {"target_alias": "Caster", "amount": {"kind": "fixed", "value": 250.0}})
    heal_result = _execute_effect_with_context(registry, damaged_state, heal_effect, command, source_suffix="fixed_heal")
    heal_after = reducer.apply_all(damaged_state, heal_result.mutations)
    cases["heal_result"] = heal_result
    cases["heal_before_state"] = damaged_state
    cases["heal_transition"] = _effect_transition(
        command=command,
        before_state=damaged_state,
        after_state=heal_after,
        effect=heal_effect,
        effect_result=heal_result,
        target_id=command.actor_id,
        coverage_id="v0_213_fixed_heal",
    )

    shield_effect = _synthetic_effect("Shield", "fixed_shield", {"target_alias": "Caster", "amount": {"kind": "fixed", "value": 300.0}})
    shield_result = _execute_effect_with_context(registry, base_state, shield_effect, command, source_suffix="fixed_shield")
    shield_after = reducer.apply_all(base_state, shield_result.mutations)
    cases["shield_result"] = shield_result
    cases["shield_before_state"] = base_state
    cases["shield_transition"] = _effect_transition(
        command=command,
        before_state=base_state,
        after_state=shield_after,
        effect=shield_effect,
        effect_result=shield_result,
        target_id=command.actor_id,
        coverage_id="v0_213_fixed_shield",
    )

    resource_effect = _synthetic_effect(
        "ResourceDelta",
        "fixed_resource_delta",
        {"target_alias": "Caster", "resource": "energy", "amount": {"kind": "fixed", "value": 10.0}},
    )
    resource_result = _execute_effect_with_context(registry, base_state, resource_effect, command, source_suffix="fixed_resource")
    resource_after = reducer.apply_all(base_state, resource_result.mutations)
    cases["resource_delta_result"] = resource_result
    cases["resource_delta_before_state"] = base_state
    cases["resource_delta_transition"] = _effect_transition(
        command=command,
        before_state=base_state,
        after_state=resource_after,
        effect=resource_effect,
        effect_result=resource_result,
        target_id=command.actor_id,
        coverage_id="v0_213_fixed_resource_delta",
    )

    unsupported_formula_effect = _synthetic_effect(
        "Heal",
        "unsupported_formula_heal",
        {"target_alias": "Caster", "amount": {"kind": "dynamic_hash", "hash": 12345}},
    )
    unsupported_scope_effect = _synthetic_effect(
        "Shield",
        "unsupported_scope_shield",
        {"target_alias": "AllTeam", "amount": {"kind": "fixed", "value": 100.0}},
    )
    cases["unsupported_formula_result"] = _execute_effect_with_context(
        registry,
        base_state,
        unsupported_formula_effect,
        command,
        source_suffix="unsupported_formula",
    )
    cases["unsupported_scope_result"] = _execute_effect_with_context(
        registry,
        base_state,
        unsupported_scope_effect,
        command,
        source_suffix="unsupported_scope",
    )
    return cases


def _execute_effect_with_context(
...
def _synthetic_effect(opcode: str, raw_id: str, standard: dict[str, Any]) -> EffectIR:
    return EffectIR(
        effect_id=f"effect:validation:{VALIDATION_VERSION}:{raw_id}:{opcode}",
        opcode=opcode,
        payload={"standard": standard, "validation_only": True},
        source=IRSource(
            source_path=f"validation/{VALIDATION_VERSION}",
            raw_type="SyntheticEffect",
            raw_id=raw_id,
            evidence={"purpose": "runtime contract validation"},
        ),
...
def _risk_guard_checks(action_transition, scope_case: dict[str, Any], duplicate_add_modifier_result) -> dict[str, object]:
    plan = action_transition.coverage.get("action_event_plan", {})
    scope_windows = []
    if scope_case.get("transition") is not None:
        scope_windows = [
            window
            for window in scope_case["transition"].to_json().get("trigger_windows", [])
            if isinstance(window, dict) and str(window.get("skipped_reason", "")).startswith("scope_mismatch")
        ]
    lifecycle_records = _effect_records_of_type(duplicate_add_modifier_result, "status_lifecycle")
    checks = {
        "action_event_plan_is_marked_as_derived": isinstance(plan, dict) and bool(plan.get("steps")),
        "scope_is_conservative_actor_or_primary_target": bool(scope_windows),
        "status_lifecycle_not_silent_replace": bool(lifecycle_records),
    }
    return {
        "ok": all(checks.values()),
        "checks": checks,
        "risk_notes": {
            "action_event_plan": "derived_from_ActionDefinitionIR_until_TBGD_ActionEventIR_exists",
            "trigger_scope": "actor_or_primary_target_only; listener scopes blocked",
            "status_lifecycle": "stack_refresh_duration still partial but lifecycle_result is mandatory",
        },
    }


def _status_lifecycle_checks(effect_result) -> dict[str, object]:
    records = _effect_records_of_type(effect_result, "status_lifecycle")
    payloads = [record.get("payload", {}) for record in records if isinstance(record, dict)]
    checks = {
        "has_lifecycle_record": bool(records),
        "has_lifecycle_plan": any(isinstance(payload.get("lifecycle_plan"), dict) for payload in payloads if isinstance(payload, dict)),
        "duplicate_is_replace_partial": any(
            payload.get("operation") == "refresh_or_replace_partial" for payload in payloads if isinstance(payload, dict)
        ),
        "partial_has_reason": any(
            any("refresh_or_replace_partial" in str(reason) for reason in payload.get("unsupported", []))
            for payload in payloads
...
def _remove_modifier_checks(transition) -> dict[str, object]:
    records = _records_of_type(transition, "status_lifecycle")
    payloads = [record.get("payload", {}) for record in records if isinstance(record, dict)]
    status_paths = [list(mutation.path) for mutation in transition.transaction.mutations] if transition is not None else []
    ...
    after_statuses = after_units.get(selected, {}).get("statuses", []) if isinstance(after_units, dict) else []
    removed_status = ""
    for payload in payloads:
        if isinstance(payload, dict) and payload.get("operation") == "remove":
            removed_status = str(payload.get("status_id") or "")
            break
    checks = {
        "has_remove_transition": transition is not None,
        "has_remove_lifecycle_record": any(payload.get("operation") == "remove" for payload in payloads if isinstance(payload, dict)),
        "removed_status_id": bool(removed_status),
        "status_id_removed": bool(removed_status) and removed_status not in after_statuses,
        "statuses_mutated": any(path[-1:] == ["statuses"] for path in status_paths),
        "status_details_mutated": any(path[-2:] == ["flags", "status_details"] for path in status_paths),
    }
    return {"ok": all(checks.values()), "checks": checks, "records": records, "after_statuses": after_statuses}


def _fixed_effect_checks(cases: dict[str, Any]) -> dict[str, object]:
    expected = {
        "heal_transition": "heal",
        "shield_transition": "shield",
        "resource_delta_transition": "resource_delta",
    }
    details: dict[str, Any] = {}
    checks: dict[str, bool] = {}
    for key, record_type in expected.items():
        transition = cases.get(key)
        records = _records_of_type(transition, record_type)
        mutations = transition.transaction.mutations if transition is not None else ()
        checks[f"{record_type}_transition_exists"] = transition is not None
        checks[f"{record_type}_has_record"] = bool(records)
        checks[f"{record_type}_has_mutation"] = bool(mutations)
        checks[f"{record_type}_record_is_mutation_linked"] = any(bool(record.get("mutation_id")) for record in records)
        details[record_type] = {"records": records, "mutations": [mutation.to_json() for mutation in mutations]}
    return {"ok": all(checks.values()), "checks": checks, "details": details}


def _unsupported_lifecycle_effect_checks(cases: dict[str, Any]) -> dict[str, object]:
    formula_reasons = list(cases.get("unsupported_formula_result").unsupported) if cases.get("unsupported_formula_result") else []
    scope_reasons = list(cases.get("unsupported_scope_result").unsupported) if cases.get("unsupported_scope_result") else []
    chance_reasons = list(cases.get("unsupported_chance_result").unsupported) if cases.get("unsupported_chance_result") else []
    checks = {
        "unsupported_formula_has_reason": any("unsupported_formula" in reason for reason in formula_reasons),
        "unsupported_scope_has_reason": any("unsupported_or_missing_target_alias" in reason for reason in scope_reasons),
        "unsupported_chance_has_reason": any("chance_unsupported" in reason for reason in chance_reasons),
    }
    return {
        "ok": all(checks.values()),
        "checks": checks,
        "reasons": {
            "formula": formula_reasons,
            "scope": scope_reasons,
            "chance": chance_reasons,
        },
    }


def _effect_transition_contract_checks(cases: dict[str, Any]) -> dict[str, object]:
    transition_validator = TransitionContractValidator()
    settlement_validator = SettlementTraceabilityValidator()
    reducer = MutationReducer()
    checks: dict[str, bool] = {}
    details: dict[str, Any] = {}
    for name in ("remove", "heal", "shield", "resource_delta"):
        transition = cases.get(f"{name}_transition")
        ...
        before_state = _state_from_transition_before(cases, name)
        replay = (
...
def _state_from_transition_before(cases: dict[str, Any], name: str) -> BattleState | None:
    transition = cases.get(f"{name}_transition")
    if transition is None:
        return None
    # The validator only needs a real BattleState for reducer replay. Store it
    # next to each transition in the lifecycle case map to keep transition JSON
    # free of non-serializable state objects.
    return cases.get(f"{name}_before_state")


def _lifecycle_case_summary(cases: dict[str, Any]) -> dict[str, Any]:
    summary: dict[str, Any] = {}
    for key, value in cases.items():
        if key.endswith("_before_state"):
            continue
        if hasattr(value, "to_json"):
            summary[key] = value.to_json()
        elif value is None:
            summary[key] = None
        else:
            summary[key] = _effect_result_json(value)
    return summary
...
def _effect_records_of_type(effect_result, record_type: str) -> list[dict[str, Any]]:
    if effect_result is None:
        return []
    return [
        record
