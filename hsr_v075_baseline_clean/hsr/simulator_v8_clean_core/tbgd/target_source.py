...
def build_target_source_projection(
    ...
    *,
    ...
    records = _target_records_from_scope(snapshot, scope_catalog)
    definitions, fingerprints = _target_language_definitions(tbgd_root)
...
def _target_language_definitions(
    ...
    names_by_kind = {
        "alias": {item[3] for item in raw_definitions if item[0] == "alias"},
        "operation": {item[3] for item in raw_definitions if item[0] == "operation"},
    }
    overlap = names_by_kind["alias"] & names_by_kind["operation"]
    definition_ids = {
        f"{kind}:{name}": _stable_id("target_language_definition", relative, kind, name)
    ...
    for kind, relative, top_key, name, raw, source, _digest in raw_definitions:
        ...
        target_names, missing = _target_language_reference_names(
            raw,
            alias_names=names_by_kind["alias"],
            operation_names=names_by_kind["operation"],
        )
    ...
    return tuple(definitions), MappingProxyType(dict(fingerprints))
                    ...
                    for token in _target_alias_reference_tokens(alias):
                        ...
                        for operation in parts[1:]:
                            if operation in operation_names:
                                references.add(f"operation:{operation}")
                                if operation in alias_names:
                                    missing.append(
