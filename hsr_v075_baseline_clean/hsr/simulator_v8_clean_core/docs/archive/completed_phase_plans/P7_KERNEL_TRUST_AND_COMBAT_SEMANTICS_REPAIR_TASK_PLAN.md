...
- [x] P7-S3 已实现所选执行图原子提交，任一选中节点失败时 state unchanged。
- [x] P7-S4 已彻底分离规则输入与审计来源，并收敛版本化 engine rule 来源。
- [x] P7-S5 已将当前可执行 target / numeric / condition 子集 lower 为类型化 IR，runtime 不再解析 raw 子语言。
- [x] P7-S6 已建立动作所有权、动作角色、使用窗口和资源门的统一查询 / 提交契约。
- [x] P7-S7 已建立目标关系、选择基数、主目标和实际作用范围契约。
- [x] P7-S8 已建立幂等查询、决策 token、一次提交一次推进和敌我统一外部控制流程。
- [x] P7-S9 已建立显式回合 / 事件阶段机，并回正 turn、DoT、状态 tick 和插入动作窗口。
- [x] P7-S10 已回正速度变化、行动值、同值顺序、控制跳过和时间线推进语义。
- [x] P7-S11 已建立队列条目终态和前进保证，无效头项不再永久阻塞。
- [x] P7-S12 已建立统一且分家族适用的伤害 / 削韧管线，并回正 before-event 计算顺序。
- [x] P7-S13 已建立一等护盾实例、吸收 / 替换语义和 HP 路由。
- [x] P7-S14 已回正状态施加、命中 / 抵抗 / 免疫、definition linking 和 partial no-mutation。
- [x] P7-S15 已建立独立随机决策身份、精确选择账本和严格 replay 消费。
- [x] P7-S16 已接通真实战斗中的召唤物 / servant spawn、presence、targetability、行动和清理生命周期。
- [x] P7-S17 已接通波次状态机、单位入场、wave event、清理和结束判定。
- [x] P7-S18 已建立推演器可用的紧凑只读状态键，并将静态 provenance / 审计详情移出搜索节点复制路径。
- [x] P7-S19 已完成问题总账继承、不变量聚合、P1-P6 串行回归、报告和长期文档收口。
- [x] P7-DONE：P7-I01 至 P7-I24 均经独立验收关闭，内核只对完整原子结果给出可信后继；P7 内容覆盖边界与全角色 / 全怪物 / 全装备 / 全关卡完成度已被诚实区分。

统一验收结论（2026-07-13）：P7-S0 至 P7-S19 全部通过独立验收。最终聚合使用当前源码和一次共享 RuleBook 生成九项必要回归，24 项问题证据、18 个阶段证据和 P1-P6 当前口径回归均无失败；空集合、缺项、错误类型及错误源码指纹负例均被拒绝，且不依赖历史临时文件。P7 内核可信执行与战斗语义回正阶段完成。P2 action-delay callback graph、P3 servant action graph 以及角色、怪物、装备和关卡内容扩面继续作为后续内容缺口保留，不属于 P7 内核不变量验收失败。
