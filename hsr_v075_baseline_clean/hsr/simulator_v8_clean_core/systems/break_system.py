...
class BreakSystem:
    ...
    def enter_break(self, state: BattleState, packet: ToughnessPacket) -> BreakApplicationResult:
        ...
        for emission in self.rules.break_damage_emissions_for_template(template.template_id):
            records.append(_break_damage_record(packet, template, emission))
            if emission.coverage_status == "executable":
                errors.append("break_damage_formula_runtime_not_admitted")
...
def _break_damage_record(
    packet: ToughnessPacket,
    template: BreakTemplateIR,
    emission: BreakDamageEmissionIR,
) -> dict[str, JSONValue]:
    reason = emission.blocked_reason or (
        "break_damage_formula_runtime_not_admitted"
        if emission.coverage_status == "executable"
        else f"break_damage_not_executable:{emission.coverage_status}"
    )
    return SettlementRecord(
