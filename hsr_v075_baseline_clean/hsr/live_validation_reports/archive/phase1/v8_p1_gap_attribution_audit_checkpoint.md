...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m simulator_v8_clean_core.tools.validate_p1_6_target_system --output-dir /tmp/hsr_v8_p1_6_after_global_target
PYTHONDONTWRITEBYTECODE=1 python3 -m simulator_v8_clean_core.tools.validate_p1_7_rng_branch_system --output-dir /tmp/hsr_v8_p1_7_after_gap_split
PYTHONDONTWRITEBYTECODE=1 python3 -m simulator_v8_clean_core.tools.validate_p1_9_phase1_aggregate --output-dir /tmp/hsr_v8_p1_9_after_gap_fix
```
...
本次已把 `TargetAliasConfig` / `TargetOperationConfig` 投影到 Canonical IR 的 `TargetExpressionIR`，并去掉 `ModifierOwnerEntity` 缺 owner 时退回 caster 的 fallback。`formation sort`、`toughness sort`、`owner fetch` 现在都有真实全局目标配置来源正例。
...
| P1-6 formation sort | source gap | 已修正为 executable | 通过 `TargetAliasConfig.AliasDict` + `TargetOperationConfig.OperationDict` 生成 `AllEnemy.SortByFormation` alias chain IR；source trace 同时记录 base alias 与 operation config。 | 保持正例和缺 position / invalid target 等负例回归。 |
| P1-6 toughness sort | source gap | 已修正为 executable | 不是 `TargetSortByStance` raw 名称，而是 `TargetOperationConfig` 中 `SortByStance` / `SortByStanceRatio` 映射到 `TargetSortByProperty(CurrentStance)` / `TargetSortByPropertyRatio(StanceRatio)`。 | 后续不要只按 raw type 名字判断是否 source gap。 |
| P1-6 owner fetch | source gap | 已修正为 executable | `TargetAliasConfig.AliasDict.ModifierOwnerEntity` 投影为 `TargetFetchModifierOwner`；验证补了缺 owner blocked 负例，`ModifierOwnerEntity` alias 不再默认退回 caster。 | 保持 owner context 缺失时 blocked/state unchanged。 |
...
- 真 source gap：`DispelStatus(Order=Random)`。
- 已修正为 executable：formation sort、toughness sort、owner fetch。
- lowering/admission/validation gap：stack+duration refresh、servant target、servant initial setup、battle_unit_summon initial setup、control resist。
...
3. `P1-GAP-2`：servant / battle_unit_summon admission 审计，拆清楚 servant、battle unit summon、summoned monster、adventure summon unit。
4. `P1-GAP-3`：RNG source matrix 拆分，把 random dispel 和 control resist 分开验收。
