# Panel Payload Rules Checkpoint v0.94
...
`derived_stat_rules_checkpoint_v0_93.md`.

Covered refactor:

- Added `stat_rules.panel_from_unit_payload()` for initial panel construction
  from canonical unit-add payloads.
- Moved reducer-local unit payload panel construction out of
  `transition_reducer.py`.
- Routed unit-add replay through the same `stat_value_from_parts()` helper used
  by runtime unit stats and snapshot stat replay.

This further reduces duplicated stat math in the transition reducer. Initial
panel construction, runtime unit stat calculation, runtime contextual stat
calculation, and replay panel refresh now share the stat-rule module.
...
python3 simulator_v7_7/hsr_simulator_prototype_v7_7.py \
  ...
  --route-mode exact \
  --output /tmp/hsr_v094_panel_payload_rules_c0_to_c8_exact.json
python3 simulator_v7_7/hsr_simulator_prototype_v7_7.py \
  --model-pack model_pack_v3_0 \
  --case-id arbitration_4_3_knight_3_live_c0_to_c8_simulator_only \
  --route-mode auto_probe \
  --output /tmp/hsr_v094_panel_payload_rules_auto_probe.json
python3 simulator_v7_7/hsr_simulator_prototype_v7_7.py \
  simulator_v7_7/examples/seele_five_dummy_case_v0_3.yaml \
  --output /tmp/hsr_v094_panel_payload_rules_seele_five_dummy.json
```
...
The stat-rule module still has separate runtime and snapshot paths for resolving
derived stat sources. They share formula handling but not a single explicit
source-resolution abstraction yet.

The broader engine still needs the same consolidation pass for non-stat rule
areas such as target resolution, status lifecycle timing, resource settlement,
and action-axis changes.
