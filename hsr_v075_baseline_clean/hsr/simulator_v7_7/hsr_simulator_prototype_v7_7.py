...
class BattleSimulator:
    def __init__(self, case: dict[str, Any], tbgd_source: Any = None):
        ...
        self._combat_runtime = self._combat_executor.bind_runtime(self)
    ...
    def record_rng_event(
        ...
        *,
        ...
        if stl is not None:
            ...
            sync = getattr((ctx or {}).get("_transaction"), "sync_from_transition", None) if isinstance(ctx, dict) else None
            if callable(sync):
                sync()
    ...
    def resolve_route_step(self, step: dict[str, Any]) -> None:
        return self._combat_executor.resolve_route_step(self, step)
    ...
    def resolve_action(self, action: dict[str, Any], targets: list[str], events: dict[str, Any], context: Optional[dict[str, Any]] = None) -> None:
        return self._combat_executor.resolve_action(self, action, targets, events, context=context)
    ...
    def resolve_break_damage_packet(self, packet: dict[str, Any], actor: UnitState, target: UnitState, action: dict[str, Any], events: dict[str, Any]) -> dict[str, Any]:
        return self._damage_resolver.resolve_break_packet(packet, actor, target, action, events)

    def _resolve_break_damage_packet_legacy(self, packet: dict[str, Any], actor: UnitState, target: UnitState, action: dict[str, Any], events: dict[str, Any]) -> dict[str, Any]:
    ...
    def resolve_super_break_damage_packet(self, packet: dict[str, Any], actor: UnitState, target: UnitState, action: dict[str, Any], events: dict[str, Any]) -> dict[str, Any]:
        return self._damage_resolver.resolve_super_break_packet(packet, actor, target, action, events)

    def _resolve_super_break_damage_packet_legacy(self, packet: dict[str, Any], actor: UnitState, target: UnitState, action: dict[str, Any], events: dict[str, Any]) -> dict[str, Any]:
        packet = self.normalize_damage_packet_schema(packet)
        element = str(packet.get("element", "none") or "none").lower()
        dmg_ctx = {"action": action, "packet": packet, "actor_id": actor.id, "actor": actor, "owner_id": actor.flags.get("owner_id"), "target_id": target.id, "target": target}
        base = coerce_float(packet.get("break_base_damage", break_base_damage(actor.level)))
    ...
    def apply_hp_loss(self, target: UnitState, amount: float, ctx: dict[str, Any], label: str = "hp_loss", ignore_shield: bool = False, carry_over_hp_bar_damage: Optional[bool] = None) -> dict[str, Any]:
        return self._damage_applier.apply_hp_loss(
            target,
            amount,
            ctx,
            label=label,
    ...
    def _apply_hp_loss_legacy(self, target: UnitState, amount: float, ctx: dict[str, Any], label: str = "hp_loss", ignore_shield: bool = False, carry_over_hp_bar_damage: Optional[bool] = None) -> dict[str, Any]:
    ...
    def resolve_dot_tick(self, unit: UnitState, ctx: dict[str, Any]) -> None:
        return self._damage_resolver.resolve_dot_tick(unit, ctx)

    def _resolve_dot_tick_legacy(self, unit: UnitState, ctx: dict[str, Any]) -> None:
    ...
    def apply_effect(self, eff: dict[str, Any], ctx: dict[str, Any]) -> None:
        return self._combat_executor.apply_effect(self, eff, ctx)
    ...
    def drain_queues(self, default_events: Optional[dict[str, Any]] = None) -> None:
        return self._combat_executor.drain_queues(self, default_events=default_events)
